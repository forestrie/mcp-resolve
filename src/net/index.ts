 ...
 * setup never sees a real network call.
 *
 ...
 * `chain.ts` makes the JSON-RPC calls for the accumulator read, through
 * `@forestrie/chain-rpc` with the injected `fetchImpl`: `readLogState` is the three-call `eth_chainId` /
 ...
 * snapshot; `readChainHead` is the same first two calls
