...
was over its daily request cap: plain text
...
answered 200 on every probe since (`test/fixtures/lane-a/PROVENANCE.md`),
so a real 429 exchange cannot be captured on demand: it
...
tool and the receipt step of `verify_fetched_receipt`, asserting the
...
Frozen alongside the captured fixtures: a real 429 capture, if one is ever
taken, replaces this file in a commit of its own, and this note is updated
to say so.
