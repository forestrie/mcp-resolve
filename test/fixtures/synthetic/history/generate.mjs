#!/usr/bin/env node
/**
 * plan-2609-06 step 1.2/1.3 — the SYNTHESISED buried-peak fixture.
 *
 * No real buried peak exists on lane A's publications log (see
 * `test/fixtures/chain/history/e8345800-a747-4e62-9409-61622b836f1f/PROVENANCE.md`):
 * the log has not grown since plan-2609-05's capture, so the verifier's own
 * receipt (mmr index 8, peak at mmr index 9) is still held by the latest
 * state. This generator fabricates a "latest" state one checkpoint newer
 * than the real history, at a size that does NOT hold that peak, so the
 * buried-peak fallback (F1/F2) has something real to walk back through.
 *
 ...
 * (plan-2609-06 step 1.2) and are never modified by this script.
 *
...
export function generate(outDir) {
  ...
  const provenance = `# SYNTHESISED — plan-2609-06 buried-peak fixture
