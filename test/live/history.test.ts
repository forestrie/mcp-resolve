 ...
 * NEVER a required check: the checkpoint-history paths, exercised against
 * the real publications log (`e8345800-a747-4e62-9409-61622b836f1f`)
 ...
 * scan needed) or has folded past it (the history fallback runs) —
...
describe.skipIf(!FORESTRIE_LIVE)("live: checkpoint history", () => {
  ...
  it(
    ...
    async () => {
      ...
      if (grew) {
        ...
        const scannedTo = structured.provenance?.history?.scannedTo as number;
